<?php
    ...
    if($filtered_content=='not'){
        $query = "SELECT * FROM APP WHERE type = '{$filtered_type}' and category = '{$filtered_id}' ORDER BY {$filtered_order} DESC" ;
    }
    else if($filtered_type =='free'){
    $query = "SELECT * FROM APP WHERE type = 'Free' and category = '{$filtered_id}' and content_rating = '{$filtered_content}'ORDER BY {$filtered_order} DESC" ;

    }else if($filtered_type=='paid'){
        $query = "SELECT * FROM APP WHERE type = 'Paid' and category = '{$filtered_id}' and content_rating = '{$filtered_content}'ORDER BY {$filtered_order} DESC" ;
    }else if($filtered_type=='all'){
        $query = "SELECT * FROM APP WHERE category = '{$filtered_id}' and content_rating = '{$filtered_content}' ORDER BY {$filtered_order} DESC";
    }
    if($filtered_type=='all' && $filtered_content=='not'){
        $query = "SELECT * FROM APP WHERE category = '{$filtered_id}' ORDER BY {$filtered_order} DESC";
    }
...
<body>
    <h2><a href="index.php">구글 플레이 스토어 검색</a> | 카테고리 조회</h2>
    <table border="1">
        <tr>
            <th>name</th>
            <th>category</th>
